...
public class FllowCam : MonoBehaviour
{
    ...
    public float roateDamping = 10f;//회전속도 계수
    ...
    {   //var 변수는 가변 자료형이라고 볼 수 있지만
        ...
        //회전할때 값을 구해보자
        tr.rotation = Quaternion.Slerp(tr.rotation, target.rotation, Time.deltaTime * roateDamping);
        //카메라가 위치 및 회전  이동 후에 타겟을 바라보도록
        tr.LookAt(target.position+(target.up*targetOffset));
         
    }
    //기즈모라는 씬뷰에서 보이는 가상의 라인들을 확인하기 위한 메소드 디버그 느낌.
    private void OnDrawGizmos()
    {//기즈모의 색상지정
        Gizmos.color = Color.green;
        //DrawWireSphere(위치,반경)
        //기즈모에는 몇가지 지정된 모양이 있음
        //DrawWireSphere는 선으로 이루어진 구형모양
        Gizmos.DrawWireSphere(transform.position+(target.up*targetOffset),0.1f);
        //메인 카메라와 대상간의 선을 표시
        //DrawLine(A위치,B위치 )=A와B사이 선 긋기
